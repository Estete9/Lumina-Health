...
export function AddPatientModal({ isOpen, onClose, onSuccess }: AddPatientModalProps) {
  ...
  const [primaryAilment, setPrimaryAilment] = useState('Generalized Anxiety Disorder');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!primaryAilment) { setErrorMsg('Primary Ailment is required.'); return; }
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
          <div className="flex-1 overflow-y-auto space-y-4 pr-1.5 py-1 [scrollbar-width:thin]">
            ...
            <div className="pt-1 border-t border-slate-100 mt-4">
              <label className="block text-xs font-semibold text-slate-700 mb-2 mt-2">
...
fs.writeFileSync('c:/Users/cepal/Documents/01_Dev/agentic_projects/antigravityPG/Lumina_Health/components/patients/AddPatientModal.tsx', content);

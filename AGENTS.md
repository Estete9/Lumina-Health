...
- **Parallel Frontend Delegation:** The Orchestrator is authorized to spawn multiple independent Frontend Engineer agents simultaneously to work on different UI tasks within the same sprint, PROVIDED that their tasks are strictly isolated to different files or components to completely avoid race conditions and merge conflicts.
...
5. **The QA Verifier (Integration Tester)**
   ...
   - **Verification Execution:** Conducts static type checking (`npx tsc --noEmit`) and runs automated unit tests (`npm run test`). **CRITICAL RULE:** Full E2E browser regression suites (`npm run test:e2e` / Playwright) MUST ONLY be executed when explicitly requested by the user at the very end of a sprint. For standard verification, the QA Verifier must ONLY run the specific newly created E2E test file for that sprint (e.g., `npx playwright test e2e/sprints/sprint17.spec.ts`). Sole agent authorized to execute terminal test commands during Gate 3.
...
4. **Phase 3: QA & Verification (Sequential Gate):** - Once BOTH implementation agents have self-terminated, spawn **The QA Verifier**.
   ...
   - **CRITICAL E2E RULE:** The QA Verifier MUST ONLY run the specific sprint's new E2E test file (e.g. `npx playwright test path/to/file.spec.ts`) during standard Gate 3 execution. DO NOT run the full global E2E regression suite (`npm run test:e2e`) unless the user explicitly requests a full regression pass.

...
- **Subagent Lifecycle & Cleanup:** Do not kill active subagents while work or QA is underway. When a subagent finishes its task and QA Verifier yields a PASS, the Orchestrator then cleans up subagent sessions using `manage_subagents` (`kill` or `kill_all`).
